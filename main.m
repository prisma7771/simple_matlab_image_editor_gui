...
% handles    structure with handles and user data (see GUIDATA)
myImago2 = getappdata(0, 'Image1');
B = str2double(get(handles.br,'String'));
B2 = myImago2 + B;
setappdata(0, 'Image2', B2);
axes(handles.img2);
hold off;
cla reset;
imshow(B2);
...
% --- Executes on slider movement.
function br_slider_Callback(hObject, eventdata, handles)
% hObject    handle to br_slider (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'Value') returns position of slider
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
br_sliderx = get(hObject,'Value');
br_sliderx = round(br_sliderx);
set(handles.br,'String',strcat(num2str(br_sliderx)));


% --- Executes during object creation, after setting all properties.
function br_slider_CreateFcn(hObject, eventdata, handles)
% hObject    handle to br_slider (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
G = [1/25 1/25 1/25 1/25 1/25;
     ...
     1/25 1/25 1/25 1/25 1/25;
     1/25 1/25 1/25 1/25 1/25];
LowpassR = uint8(convn(double(RImage), double(G),'same'));
LowpassG = uint8(convn(double(GImage), double(G),'same'));
LowpassB = uint8(convn(double(BImage), double(G),'same'));
HighpassR = RImage - LowpassR;
HighpassG = GImage - LowpassG;
HighpassB = BImage - LowpassB;
Highpass(:,:,1) = HighpassR;
Highpass(:,:,2) = HighpassG;
Highpass(:,:,3) = HighpassB;
Isharp = (A-1)*myImago2 + Highpass;
setappdata(0, 'Image2', Isharp);
axes(handles.img2);
hold off;
cla reset;
imshow(Isharp);
...
% --- Executes on slider movement.
function sp_slider_Callback(hObject, eventdata, handles)
...
% Hints: get(hObject,'Value') returns position of slider
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
sp_sliderx = get(hObject,'Value');
set(handles.sp,'String',strcat(num2str(sp_sliderx)));
